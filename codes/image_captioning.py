...
#gunera  ##show_trainig_example(train_img_fns, train_captions, example_idx=142)
...
from collections import Counter  #my code import counter word count
...
def generate_vocabulary(train_captions):
    ...
    """
    ###My code-(first flatten the sentence according to above criteria and input train_caption)
    
    flattened_sentence = [sentence for caption in train_captions for sentence in caption]
    flattened_sentence = split_sentence(' '.join(flattened_sentence))
    counter = Counter(flattened_sentence)
    vocab = [token for token, count in counter.items() if count>=5] ### YOUR CODE HERE ###
...
# 
# crunch LSTM through all the tokens, but we will ignore padding tokens during loss calculation.
...
# make sure for using correct argument in caption_tokens_to_indices
...
# 
# During training you will feed ground truth tokens into the lstm to get predictions of next tokens. 
# 
# Notice that you don't need to feed last token (END) as input (http://cs.stanford.edu/people/karpathy/):
# 
# <img src="images/encoder_decoder_explained.png" style="width:50%">
print("----------------------------------------------------------------------------")
print("***Set (finished_epoch) to (0) for starting training from the beginning  ***")
print("*********it was set to 23 to use already trained weights********************")
print("----------------------------------------------------------------------------")
...
finished_epoch = 23  #gunera !!! for training ''finished_epoch must be set to (0) zero''  ''must be revised!''


# you can load trained weights here
# you can load "weights_{epoch}" and continue training
# uncomment the next line if you need to load weights
saver.restore(s, os.path.abspath("../trained_RNN_weights/weights_{}".format(finished_epoch)))
...
from ipywidgets import IntProgress  #gunera for tqdm_notebook work properly

for epoch in range((finished_epoch+1),n_epochs):
    ...
    for _ in pbar:
        ...
        if counter == 100: print("%d th epoch (epoch internal counter: %d) Train loss: %f" % (epoch, counter, train_loss/counter)) 
    ...
    saver.save(s, os.path.abspath("../AVDOSI/trained_weights/weights_{}".format(epoch)))
...
# Now it's time to find 10 examples where RNN model works good! 
# 
...
# examples can be extended to imge captioning: to camera captured images
print("----------------------------------------------")
print("***10 randomly chosen img captioning example**")
print("----------------------------------------------")
...
#10 positive example
idx_pozitive_eval=[338,368,376,196,115,21,328,47,24,352]
#idx_negative_eval=[355,317,153,7,49,66,43,220,133,216]
print("--------------------------------------------")
print("****10 pozitive img captioning example******")
print("--------------------------------------------")
for idx in idx_pozitive_eval:
    show_valid_example(val_img_fns,example_idx=idx)
    time.sleep(1)
'''    
print("---------------------------------------------") 
print("*****10 negative img captioning example******")
print("---------------------------------------------")
for idx in idx_negative_eval:
    show_valid_example(val_img_fns,example_idx=idx)
    #print(idx)
    time.sleep(1)
